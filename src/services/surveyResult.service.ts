  ...
  static async submitSurveyResult(data: {
    ...
    return await prisma.$transaction(async (tx) => {
        ...
        const questionMap = new Map(
            survey.Question.map((q) => [q.questionId.toString(), q])
        );

        for (const answer of data.answers) {
            const question = questionMap.get(answer.questionId.toString());
            if (!question) {
                throw new Error(`Question ${answer.questionId} does not belong to survey ${data.surveyId}`);
            }
            const optionExists = question.QuestionOption.some(opt => opt.optionId === answer.optionId);
            if (!optionExists) {
                throw new Error(`Option ${answer.optionId} does not belong to question ${answer.questionId}`);
            }
        }
